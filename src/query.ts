// ═══════════════════════════════════════════════════════
// query.ts — Smart collection query resolver
//
// Query syntax:
//   1-50          → ID range
//   5,12,44       → ID list
 ...
 */
export function resolveAtom(q: string, questions: Question[]): Set<string> {
  ...
  if (rangeM) {
    ...
    questions.forEach(qq => {
      const n = parseInt(qNum(qq.id), 10);
 ...
 */
export function resolveQuery(query: string, questions: Question[]): Set<string> {
  const q = query.trim();
