// ═══════════════════════════════════════════════════════
// CollectionsModal.tsx — Collection manager with smart query builder
// ═══════════════════════════════════════════════════════

import React, { useState, useEffect, useRef } from 'react';
import { I } from './Icons';
import { Question, Collection, CollectionSet, SM2Card } from '../types';
    ...
    >
      ...
      {editing ? (
        ...
        <span
          ...
          title={`Double-click to rename: ${c.name}`}
        >
          {c.name}
        </span>
      )}
...
export interface QueryEntryRowProps {
  entry: QueryEntry;
  questions: Question[];
  onRerun: () => void;
  onDelete: () => void;
    ...
    >
      ...
      <span
        style={{
          ...
          padding: '2px 5px',
          borderRadius: 4,
          color: modeColors[entry.mode] || 'var(--primary)',
          border: `1px solid ${modeColors[entry.mode] || 'var(--primary)'}`,
          flexShrink: 0,
          fontFamily: 'var(--mono)'
        }}
      >
...
export interface CollectionsModalProps {
  questions: Question[];
  collections: (Collection & { queries?: QueryEntry[] })[];
  setName: string;
  systemSets: CollectionSet[];
  onSave: (newColls: Collection[], newName?: string, silent?: boolean) => void;
  onClose: () => void;
...
export function CollectionsModal({
  ...
  const [colls, setColls] = useState<(Collection & { queries?: QueryEntry[] })[]>(() => {
    const c = JSON.parse(JSON.stringify(collections || []));
    return c.map((col: any) => ({ ...col, queries: col.queries || [] }));
  });
  ...
  const [queryPreviews, setQueryPreviews] = useState<Record<string, Set<string>>>({});
  ...
  useEffect(() => {
    ...
    onSave(colls, editedSetName, true);
  }, [colls, editedSetName, onSave]);
  ...
  const toggleQ = (collId: string, qId: string) => {
    setColls(prev =>
      prev.map(c =>
        c.id !== collId
          ? c
          : {
              ...c,
              qIds: c.qIds.includes(qId) ? c.qIds.filter(x => x !== qId) : [...c.qIds, qId]
            }
      )
    );
  };

  const handleQueryInput = (collId: string, val: string) => {
    setQueryInputs(prev => ({ ...prev, [collId]: val }));
    const matched = resolveQuery(val, questions);
    setQueryPreviews(prev => ({ ...prev, [collId]: matched }));
  };

  const saveQueryEntry = (collId: string, text: string, mode: 'add' | 'replace' | 'remove') => {
    setColls(prev =>
      prev.map(c => {
  ...
  const applyQuery = (collId: string) => {
    ...
    setColls(prev =>
      prev.map(c =>
        c.id !== collId
          ...
          : {
              ...c,
              qIds: Array.from(new Set([...c.qIds, ...matched]))
            }
      )
    );
  ...
  const replaceQuery = (collId: string) => {
    const text = queryInputs[collId] || '';
    const matched = queryPreviews[collId] || resolveQuery(text, questions);
    setColls(prev =>
      prev.map(c => (c.id !== collId ? c : { ...c, qIds: Array.from(matched) }))
    );
  ...
  const removeQuery = (collId: string) => {
    const text = queryInputs[collId] || '';
    const matched = queryPreviews[collId] || resolveQuery(text, questions);
    setColls(prev =>
      prev.map(c =>
        c.id !== collId
          ...
          : {
              ...c,
              qIds: c.qIds.filter(id => !matched.has(id))
            }
      )
    );
  ...
  const rerunEntry = (collId: string, entry: QueryEntry) => {
    const matched = resolveQuery(entry.text, questions);
    setColls(prev =>
      prev.map(c => {
        if (c.id !== collId) return c;
  ...
  const deleteEntry = (collId: string, ts: number) => {
    setColls(prev =>
      prev.map(c =>
        c.id !== collId
          ? c
          : {
              ...c,
              queries: (c.queries || []).filter(q => q.ts !== ts)
            }
      )
    );
  };

  const editEntry = (collId: string, ts: number, newText: string) => {
    setColls(prev =>
      prev.map(c =>
        c.id !== collId
          ? c
          : {
              ...c,
              queries: (c.queries || []).map(q => (q.ts === ts ? { ...q, text: newText } : q))
            }
      )
    );
  };
  ...
  const PRESETS = [
    {
      ...
      color: '#f06058',
      icon: '⚡',
      desc: 'Questions answered wrong ≥3×',
      query: '',
      fn: () => Array.from(wrongIds)
    },
    {
      name: 'Favourites',
      color: '#f5a623',
      icon: '★',
      desc: 'Starred questions',
      query: '',
      fn: () => Array.from(favIds)
    },
    {
      name: 'Completed',
      color: '#3ecf8e',
    ...
    },
    {
      ...
      desc: 'Not yet done',
      query: '',
      fn: () => questions.filter(q => !completedIds.has(q.id)).map(q => q.id)
    },
    {
      name: 'Has Explanation',
      color: '#a78bfa',
      icon: '📖',
      desc: 'Questions with explanation text',
      query: 'exp:.',
      fn: () => questions.filter(q => q.explanation && q.explanation.trim()).map(q => q.id)
    },
    {
      name: 'MCQ Only',
      color: '#38bdf8',
      icon: '☑',
      desc: 'Questions with options (MCQ)',
      query: '',
      fn: () => questions.filter(q => q.options && q.options.length > 0).map(q => q.id)
    },
    {
      name: 'First Half',
      color: '#fb923c',
      icon: '①',
      desc: `Questions 1–${Math.floor(questions.length / 2)}`,
      query: `1-${Math.floor(questions.length / 2)}`,
      fn: () => questions.slice(0, Math.floor(questions.length / 2)).map(q => q.id)
    },
    {
      name: 'Second Half',
      color: '#e879f9',
      icon: '②',
      desc: `Questions ${Math.floor(questions.length / 2) + 1}–${questions.length}`,
      query: `${Math.floor(questions.length / 2) + 1}-${questions.length}`,
      fn: () => questions.slice(Math.floor(questions.length / 2)).map(q => q.id)
    }
  ];

  const [activeTab2, setActiveTab2] = useState<'mine' | 'system' | 'presets'>('mine');
  const qInput = (active && queryInputs[active]) || '';
  const qPreview = (active && queryPreviews[active]) || new Set<string>();
      ...
      <div className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: 760, width: '95vw' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button className="btn primary" onClick={() => onSave(colls, editedSetName)}>
              Save All
            </button>
        ...
        <div style={{ display: 'flex', gap: 2, marginBottom: 14, borderBottom: '1px solid var(--border)', paddingBottom: 0 }}>
          {[
            ...
            <button
              key={t.id}
              onClick={() => {
                setActiveTab2(t.id as any);
                if (t.id !== 'mine') setActive(null);
              }}
              style={{
                padding: '6px 14px',
                fontSize: 12,
                ...
                background: 'none',
                color: activeTab2 === t.id ? 'var(--primary)' : 'var(--muted)',
        ...
        {activeTab2 === 'mine' && (
          <>
            ...
            <div style={{ display: 'flex', gap: 12, minHeight: 400 }}>
              ...
              >
                {colls.length === 0 && (
                  <div style={{ fontSize: 12, color: 'var(--muted)', textAlign: 'center', padding: '30px 0' }}>
                    No collections yet.<br />
                    Create one above or<br />
                    clone from Presets tab.
                  </div>
                )}
                {colls.map((c, idx) => (
                  <CollectionRow
                  ...
                  <>
                    ...
                    <div style={{ background: 'var(--card2)', border: '1px solid var(--border2)', borderRadius: 10, padding: 12 }}>
                      ...
                      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 8 }}>
                        {[
                          ...
                          <button
                            key={ex}
                            onClick={() => handleQueryInput(active, ex)}
                            style={{
                              padding: '2px 8px',
                              borderRadius: 5,
                              fontSize: 10,
                      ...
                      <div style={{ fontSize: 10, color: 'var(--muted)', lineHeight: 1.7, marginBottom: 6, borderTop: '1px solid var(--border)', paddingTop: 6 }}>
                        ...
                        <code style={{ fontFamily: 'var(--mono)', color: 'var(--text)' }}>fever IN 1-50</code> ·{' '}
                        <code style={{ fontFamily: 'var(--mono)', color: 'var(--text)' }}>A &amp; B</code> AND ·{' '}
                        <code style={{ fontFamily: 'var(--mono)', color: 'var(--text)' }}>A ; B</code> OR ·{' '}
                        <code style={{ fontFamily: 'var(--mono)', color: 'var(--text)' }}>q: ans: tag: lesson:</code> fields
                      </div>
                      {qInput.trim() && (
                        <div
                          style={{
                            fontSize: 12,
                            color: previewCount > 0 ? 'var(--green)' : 'var(--red)',
                            marginBottom: 8,
                            fontWeight: 600
                          }}
                        >
                            ...
                            return qq ? (
                              <div key={qid} style={{ fontSize: 11, color: 'var(--muted)', display: 'flex', gap: 6 }}>
                                ...
                                <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                  {qq.question.slice(0, 65)}
                                  {qq.question.length > 65 ? '…' : ''}
                                </span>
                              </div>
                            ) : null;
                          })}
                      ...
                      <div style={{ display: 'flex', gap: 6 }}>
                        <button
                          ...
                          onClick={() => applyQuery(active)}
                          disabled={!qInput.trim() || previewCount === 0}
                        >
                          + Add
                        </button>
                        <button
                        ...
                        <button
                          className="btn"
                          style={{ fontSize: 12, flex: 1, color: 'var(--red)', borderColor: 'var(--red)' }}
                          onClick={() => removeQuery(active)}
                          disabled={!qInput.trim() || previewCount === 0}
                        >
                          − Remove
                        </button>
                    ...
                    <div style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.1em', color: 'var(--muted)' }}>
                      ...
                      {activeCol.qIds.length > 0 && (
                        <button
                          onClick={() => setColls(prev => prev.map(c => (c.id !== active ? c : { ...c, qIds: [] })))}
                          style={{
                            marginLeft: 10,
                            fontSize: 10,
                            color: 'var(--red)',
                        ...
                        return (
                          ...
                          >
                            ...
                            <span style={{ fontSize: 12, lineHeight: 1.5, flex: 1 }}>
                              ...
                              {q.question.slice(0, 75)}
                              {q.question.length > 75 ? '…' : ''}
                            </span>
                          </label>
                        );
                      })}
          ...
          <div style={{ minHeight: 420, overflowY: 'auto' }}>
            {!systemSets || systemSets.length === 0 ? (
              <div style={{ textAlign: 'center', padding: 60, color: 'var(--muted)', fontSize: 13 }}>
                No system collections available yet.<br />
                ...
                <div key={sset.id} style={{ marginBottom: 20 }}>
                  ...
                  >
                    ...
                    <span style={{ fontWeight: 400, fontSize: 10, color: 'var(--muted)' }}>
                      ({sset.colls.length} collections · auto-generated)
                    </span>
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                    {sset.colls.map(c => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <button
                            className="btn"
                            style={{ fontSize: 11, padding: '3px 10px', opacity: already ? 0.7 : 1 }}
                            ...
                            onClick={() => {
                              ...
                              const cloned = {
                                ...
                                color: c.color,
                                qIds: [...c.qIds],
                                queries: [{ text: `lesson:${c.name}`, mode: 'add' as const, ts: Date.now() }]
                              };
        ...
        {activeTab2 === 'presets' && (
          <div style={{ minHeight: 420, overflowY: 'auto' }}>
            <div style={{ fontSize: 11, color: 'var(--muted)', marginBottom: 14, lineHeight: 1.6 }}>
              One-click collection templates based on your current progress. Clicking creates a new collection in{' '}
              <strong>My Collections</strong> and switches to it.
            </div>
                  ...
                  >
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 2 }}>
                        <span style={{ fontFamily: 'var(--mono)', fontSize: 13, fontWeight: 700, color: p.color }}>
                          {ids.length}
                        </span>
                    ...
                    <div style={{ display: 'flex', gap: 6 }}>
                      <button
                        ...
                        onClick={() => {
                          const id = Date.now().toString();
                          const entry = p.query ? [{ text: p.query, mode: 'add' as const, ts: Date.now() }] : [];
