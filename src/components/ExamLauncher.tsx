// ═══════════════════════════════════════════════════════
// ExamLauncher.tsx — Exam mode configuration and scope selector
// ═══════════════════════════════════════════════════════

import React, { useState, useMemo } from 'react';
import { Question, Collection } from '../types';

export interface ExamLauncherProps {
  questions: Question[];
...
export function ExamLauncher({
  ...
  const [scopeMode, setScopeMode] = useState<'all' | 'collection' | 'weak'>('all');
  const [selectedCollId, setSelectedCollId] = useState<string | null>(collections[0]?.id || null);
  const [maxQs, setMaxQs] = useState(40);
  const [randomize, setRandomize] = useState(true);

  // Bugfix: Keep string IDs instead of parseInt which broke string-prefixed IDs like "GIT-Enhanced__42"
  const weakIds = useMemo(
    () => new Set(Object.entries(wrongCounts).filter(([, c]) => c >= 3).map(([id]) => id)),
    [wrongCounts]
  );
  ...
  const pool = useMemo(() => {
    ...
    return qs;
  }, [questions, collections, completedIds, excludeSolved, scopeMode, selectedCollId, weakIds]);
      ...
      <div className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: 480 }}>
            ...
            {[
              ...
              <label
                ...
                style={{
                  ...
                  padding: '9px 12px',
                  borderRadius: 8,
                  border: `1px solid ${scopeMode === opt.id ? 'var(--primary)' : 'var(--border2)'}`,
                  background: scopeMode === opt.id ? 'var(--pg)' : 'var(--card2)',
                  cursor: 'pointer',
                  transition: 'all .15s'
                }}
              >
                <input
                  ...
                  onChange={() => setScopeMode(opt.id as any)}
          ...
          {scopeMode === 'collection' && collections.length > 0 && (
            <div style={{ marginTop: 8, display: 'flex', flexWrap: 'wrap', gap: 6 }}>
              {collections.map(c => (
                ...
                >
                  {c.name} <span style={{ opacity: 0.6, fontSize: 10 }}>({c.qIds.length})</span>
                </button>
              ))}
            </div>
          )}
        ...
        <div style={{ display: 'flex', gap: 10, marginBottom: 20, alignItems: 'center' }}>
          ...
          >
            ...
            <input
              ...
              max={200}
              step={5}
              onChange={e => setMaxQs(Math.max(5, Math.min(200, parseInt(e.target.value, 10) || 40)))}
              style={{
                width: 56,
        ...
        <div style={{ padding: '10px 14px', borderRadius: 8, border: '1px solid var(--border2)', background: 'var(--card2)', marginBottom: 16, fontSize: 13 }}>
          {allSolved ? (
            ...
            <span>
              <b style={{ color: 'var(--primary)', fontFamily: 'var(--mono)', fontSize: 15 }}>
                {Math.min(finalPool.length, maxQs)}
              </b>
              <span style={{ color: 'var(--muted)' }}>
                {' '}question{finalPool.length !== 1 ? 's' : ''} will be in this exam
