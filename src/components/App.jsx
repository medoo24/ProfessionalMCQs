...
async function discoverDataFiles() {
  // ── Attempt 1: parse HTTP directory listing ──────────────────────────────
  try {
    const r = await fetch('data/');
    if (r.ok) {
      const html = await r.text();
      // Parse <a href="..."> links out of the directory index
      const links = [...html.matchAll(/href="([^"?#]+)"/gi)]
        .map(m => m[1])
        .filter(href => {
          // Keep only files (not ../ parent links or subdirs ending with /)
          if (href.endsWith('/') || href.startsWith('?') || href === '../') return false;
          const ext = href.split('.').pop().toLowerCase();
          return DATA_FILE_EXTS.has(ext);
        })
        .map(href => 'data/' + href.split('/').pop()); // normalise to data/filename
...
function App() {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      {showNarration && <NarrationMode questions={visibleQuestions} displaySettings={displaySettings} onClose={() => setShowNarration(false)} />}
